...
public class FriendInfoHandler {
	...
	/*
	 ...
	 생성한다. 클래스가 상속관계에 있다면 상위 클래스 배열에
	...
	public FriendInfoHandler( int num ) {
		/*
		 핸들러 클래스의 인스턴스를 생성할 때 데이터 저장을 위한 Friend
		 타입의 배열의 인스턴스도 함께 생성한다. 크기는 num으로 지정한다. 
		 */
	...
	public void addFriend( int choice ) {
		...
		/*
		 1,2번을 구분해서 각 인스턴스를 생성 후 저장은 부모 타입인
		...
		/*
		 High, Univ타입의 인스턴스를 Friend타입의 배열에 추가 할때에는
	...
	public void showAllData() {
		...
		for( int i = 0; i < numOfFriends; i++ ) {
			...
			/*
			 ...
			 있는지 확인하여  true/false 를 반환해주는 연산자
	...
	public void deleteInfo() {
		...
		if( deleteIndex == -1 ) {
			...
			// -1를 유지한다면 삭제된 인스턴스가 없는것으로 간주
			System.out.println("##삭제된 데이터가 없습니다.");
						
		}else {						
			// 삭제가 되었다면 해당 인덱스부터 마지막까지 반복해서...
