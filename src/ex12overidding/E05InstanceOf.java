...
/*
  ...
  -객체간에 형변환이 되려면 반드시 두 클래스간에 상속 관계가 있어야 한다.
  -부모타입의 참조변수로 자식 인스턴스를 참조할 수 있으므로 매개변수를
  부모타입으로 만들면 자식 인스턴스를 인수로 받을수 있다.
...
public class E05InstanceOf {
	...
	/*
	  ...
	  한꺼번에 받을수 있다.
	  Box타입으로 인수를 받으므로 자동형 변화(업캐스팅)이 되어 자식 클래스의
	  멤버메서드를 호출할 수 없게 된다.
	...
	/*
	 만약 부모타입의 매개변수로 자식타입의 인수를 받을수 없다면
	 아래와 같이 각 타입에 따른 모든 메서드를 정의해야 한다. 
	 */
	...
	*/
	...
	/*
	 상속관게가 더 깊어진다면 메서드는 점점 늘어나게되어 코드는 더욱 더
	 복잡해지게 돌것이다. 따라서 부모로 자식을 참조할 수 없는것은 
	 개발자에게 편리함을 가져다 준다. 
	 */
