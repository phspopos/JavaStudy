...
public class E04OverrideApply {
	...
	public static void main(String[] args) {
		...
		/*
		  err을 상요하면 콘솔레 빨간색으로 표시된다. 주로 디버깅용으로
		  사용하거나 출력문장을 구분할때 사용한다.
		 */
		...
		/*
		  ...
		  메서드가 부모쪽에 정의된 메서드를 가리게된다. 따라서 오버라이딩된
		  메서드를 호출할때는 참조변수의 영향을 받지 않는다.		  
		 */
	 ...
	 */
		...
		parent.printParent();
		/*
		 부모클래스의 참조변수로 자식 인스턴스를 참조하면 부모의
		 영역까지만 접근할 수 있으므로 자식 영역에 정의된 메서드는 
		 호출할 수 없다. 
		 */
		//parent.study(); //에러발생
		//((DeChild)parent).study();		
		parent.walk();
		//parent.walk(25);
