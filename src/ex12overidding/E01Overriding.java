...
/*
  ...
  완전히 동일한 모양으로 메서드를 재정의 하는 것을 말한다.
  오버로딩과 다른점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히
  똑같은 형태로 정의된다는 것이다
  하위클래스에서 오버라이딩한 메서드는 상위 클래스의 메서드를 가리게 되므로
  항상 하위 클래스에서 오버라이딩한 메서드가 호출된다. 
 */

//부모크래스 : 일반적인 스피커르 추상화한다.
class Speaker{
...
public class E01Overriding {
	public static void main(String[] args) {
		...
		/*
		자식 인스턴스를 생성했으므로 부모의 멤버와 자식읨 멤버 모두 
		접근할 수 있다.		  
		 */
		...
		/*
		  단 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수있는
		  범위가 부모인스턴스로 제한 된다. 따라서 자식쪽에 정의된 setBase()
		  는 호출할 수 없다.
		 */
		speaker.setVolumn(30);
		//speaker.setBase(40);
		//((BaseSpeaker)speaker).setBase(50);
		/*
		 접금범위는 앞에서 설명한 바와 같이 부모까지 제한되지만
