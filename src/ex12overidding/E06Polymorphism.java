...
/*
 ...
 말한다.
  ...
  -부모타입의 참조변수로 자식이스턴스를 참조
  1. 이 경우 부모타입의 참조변수는 부모로부터 상속받은 멤버까지만 접근할 수
  있다.
  2.자식에서 오버라이딩한 메서드가 우선적으로 호출된다.
  3.자식에서 새롭게 정의한 맴버는 접근할 수 없다.
  4.이것을 이질화(Herterogeneous:헤테로지니어스)라고 표현한다.
  
  -동일한 타입의 참조변수로 인스턴스를 참조
  1.인스턴스 전체를 접근할 수 있다. 즉 클래스의 일반적인 규칙을 따른다.
  2.이것을 동지화(Homogeneous:호모지니어스)라고 표현한다.
...
public class E06Polymorphism {
	...
	public static void main(String[] args) {
		...
		myChild.parentMethod();	//오버라이딩한 메서드
