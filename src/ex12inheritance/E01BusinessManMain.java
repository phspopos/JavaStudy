...
/*
 ...
 정의하는 것을 말한다. 부모클래스를 자식클래스가 상속할때  extends 
 를 사용한다. 
 */
...
class Man{
	...
	//멤버변수 private이므로 클래스 내부에서만 접근 가능 
	private String name;
	
	//생성자는 오버로딩으로 2개를 정의
	public Man() {
		System.out.println("Man클래스 디폴트 생성자 호출됨 111");
	}
	
	//각 생성자는 print()통해 호출여부를 확인할 수 있다.
	public Man( String name ) {
		this.name = name;
		System.out.println("Man 클래스 인자 생성자 호출됨 222");
	}
	
	//멤버메서드
	public void tellYourName() {
		//private 멤버라도 동일 클래스 내에서는 접근 가능
		System.out.println("내 이름은 " + name + " 입니다. 333");
	}	
...
class BusinessMan extends Man{
	...
	public BusinessMan( String name, String company, String position ) {
		/*
		  ...
		  생성자를 호출해 준다. 만약 super()를 기술하지 않으면 컴파일러에
		  의해 자동 삽입되어 디폴트 생성자를 호출하게 된다.
		 */
		
		//부모클래스의 매개변수 1개인 생성자 호출 (메모리에 먼저 생성됨)
		super(name);		
		//super();
		//남은 2개를 통해 자식쪽으로 멤버변수를 초기화한다.
		this.company = company;
		this.position = position;
		System.out.println("BusinessMan 생성자 호출444");
	}
	...
	public void tellYourInfo() {
		...
		상속받은 자식클래스에 접근할 수 없다. privat멤버는 무조건
