...
/*
  ...
  -노트북은 컴퓨터의 일종이다.
  -휴대폰은 전화기의 일종이다.
  이와 같이 Is-A관계가 성립하는 객채를 상속으로 표현하기에 적합하다.
...
class Computer{
	...
	/*
	  ...
	  상속받은 클래스에서 접근이 불가능하므로, public 으로 선언된
...
class NotebookComputer extends Computer{
	...
	public NotebookComputer( String name, int initCharge ) {
		/*
		  ...
		  앞에 다른 문자이 먼저 기술되면 에러가 발생된다. 항상 부모의
		  생성자를 최우선으로 호출해야한다.
		 */
		super( name );
	...
	public void movingCal() {
		...
		//사용전 베터리를 확인한 후 사용여부를 판단
		if( battery < 1 ) {
			/*
			  방전된 상태라면 사용이 불가능하므로 return을 통해 해당
			  메서드의 실행을 중지한다.
			 */
		...
		/*
		  ...
		  public으로 선언된 메서드를 통해 간접 호출해야 한다.
		 */
...
class TabletNoteBook extends NotebookComputer{
	...
	//테블릇 사용을 위한 베터리 체크 및 등록된 펜인지 확인
	public void write( String penInfo ) {
		...
		if( registPencil.compareTo(penInfo) != 0 ) {
			...
			/*
			  부모 클래스에 정의된 메서드를 호출한다. 상속받은 자식 클래스
