...
/*
  멤버변수가 상속관계에 있다 하더라도 privateㄹ으로 선언되면 클래스
   ...
   */
	...
	/*
	 ...
	 않는다. 생서자를 정의하지 않은 경우에만 자동으로 추가된다.
	 */
...
class SavingAccount extends Account{
	...
	/*
	  인자생서자에서 부모의 생성자를 호출하기 위한 super(x)를 사용한다
	  이때 매개변수가 하나인 부모클래스의 생성자가 호출된다.
	  현재 부모클래스에서는ㄴ 매개변수가 하나인 생성자가 유일하므로 만약 아래와
	  같이 호출하지 않으면 에러가 발생한다.
	 */
	public SavingAccount( int initVal ) {
		/*
		  이 문장을 삭제하면 에러가 발생하게 된다. 삭제하는 경우 super()
		  라는 문장, 즉 매개변수가 없는 디폴트 생성자를 호출하는 문장이
		  자동으로 추가되는데, 부모클래스에는 이런 형탱의 생성자가
		  없으므로 에러가 발생하는것이다.
		 */		 
		super(initVal);
