...
-컴파일시 외부클래스$클래스명.class 파일이 생성된다.
...
class Unit{
	...
	class Marine {
		...
		int attackPower;
	/*
	 내부클래스에는 외부클래스의 모든 멤버에 접근할 수 있다
...
public class E01InnerClassMain {
	...
	public static void main(String[] args) {
		...
		/*
		 Medic은 내부에서 생성되지 않았으므로 new를 두번 사요해서
		 인스턴스르 생성하면 된다. 
		 */
		//Unit.Medic medic = new Unit("메딕", 30).new Medic();
		//medic.heal();
		
		Unit unit = new Unit("커멘드센트", 1000);
		System.out.println( unit.infoStr() );
		
		Unit.Marine marine = new Unit( "병사", 80 ).marine;
		marine.attak();
		
		Unit.Medic medic = new Unit("간호사", 50).new Medic();
		medic.heal();
