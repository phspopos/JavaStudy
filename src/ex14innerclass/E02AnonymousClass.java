...
/*
  ...
  형식]
  		부모클래스명 참조변수 = new 부모클래스(){
  			익명클래스의 실행부;
  			부모클래스의 메서드 오버라이딩();
  		};
  		
  		※부모클래스를 상속받아 이름없는 자식클래스를 생성하는 개념으로
  		이해하면 된다. 상속되기 때문에 오버라이딩이 가능하다.
  		즉 중괄호 안쪽은 "자식"의 영역이다.
 */
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		 GirlGroup 클래스를 상속받아 익명 클래스 정의
		 : Singer타입의 변수에 GirlGroup을 상속받은 익명클래스를
		 생성해서 그 주소값을 할당한다.
		 개념적으로 new  GirlGroup(){}
		 	=> 익명 extends GirlGroup 과 같다
		 	이름이 업는 인스턴스이므로 부모클래스의 이름을 빌려서 자식의
		 	영역을 만든다고 생각하면 된다.
		 */
		...
		Singer ss = new GirlGroup("김하나", "블링블링") {
			
			@Override
			public String toString() {
				return super.toString() + "ss 실행 되나";
			}
		...
		ISinger sing = new ISinger() {
			@Override
			public void dancing() {
				System.out.println("춤을 추는게 가능");
			}
		};
...
/*
 ...
 제한 된다. 따라서 아래와 같이 자식에서 확장한 멤버변수는 
 접근할 수 없어 에러가 발생한다.
 ...
 */
		//System.out.println(s3.mic); //에러 발생됨
		...
		Singer sin = new Singer("김두나") {			
			@Override
			public void test() {
				System.out.println("오버라이딩 되나");
			}
		};
