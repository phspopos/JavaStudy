...
/*
  ...
  -자바는 단일 상속만 원칙으로 하지만 인터페이스를 통해 다중 상속을 구현
  할 수 있다.
  -추상클래스와 동일하게 상속을 목적으로 제작되므로 구현받은 하위 클래스
  에서는 반드시 추상메서드를 오버라이딩 행야한다.
  
  -멤버로는 추상메서드와 상수만 선언할 수 있다.
   	메서드 : public abstract
   	상 수 : public static final   	
...
interface MyInterface1 {
	...
	/*
		멤버변수 : 인터페이스에 선언되는 변수는 상수로 선언된다. 따라서
		대문자로 작성해야 한다.
	*/
...
/*
  멤버메서드 : 메서드는 선언시 추상메서드가 된다. 따라서 public 
  abstract는 생략한는 것이 좋다. 
 */
...
/*
  ...
  또한 상속과 구현을 동시에 사용할수 있다.
 */
class MyClass extends SimpleClass implements MyInterface1, MyInterface2 {
	
	/*
	  인터페이스에 동일한 이름의 추상메서드가 존재한는 경우 하나만
	  오버라이딩 하면 된다. 오버라이딩은 항상 자식쪽의 메서드가 호출되므로
	  아무런 문제도 발생하지 않는다.
	 */
	@Override
...
public class E03InterfaceMain {
	...
	public static void main(String[] args) {
		...
		/*
		  MyClass의 부모는 모두 3개 이므로 아래는 모두 참조 가능하다.
		 */
		...
		System.out.println("=====================");
		System.out.println( MyInterface1.MAX_INT );
		System.out.println( MyInterface1.MAX_DBL );
		
		/*
		System.out.println( ((SimpleClass)my1).simple );
		((SimpleClass)my1).mySimple();		
		*/
		
		
		/*
		MyInterface1 face = new MyClass();
		face.absFunc1();
		face.absFunc2();
		
		((MyClass)face).mySimple();
		*/
