...
/*
  ...
  -인스턴스 생성은 할수 없지만 그 외 참조변수, 상속 등의 모든 기능을 
  사용할수 있다.
  
  추상메서드
  - 오버라이딩을 목적으로 제작된다.
  - 메서드의 실행부가 없으므로 중괄호 대신 세미콜론(;)으로 마무리 한다.
...
/*
 추상메서드를 포함하지 않고 추상클래스를 상속하면 오버라이딩은 강제사항이 
...
/*
 ...
 클래스는 반드시 abstract로 선언해야 한다. 그렇지 않으면 에러가 
 발생한다. 
 */
...
/*
  ...
  해당 클래스도 abstract로 선언해야한다. 단, 이 경우 해당 클래스로도
  인스턴스를 생성할 수 없게된다. 따라서 인스턴스 생성을 위해서는 부모의
  추상메서드를 자식쪽에서 오버라이딩 하면 된다.
 */
class AbsClassChild2 extends AbsClass2{
	
	/*
	 오버라이딩은 부모쪽 메서드를 가리고, 새로운 기능을 재정의 하는 
