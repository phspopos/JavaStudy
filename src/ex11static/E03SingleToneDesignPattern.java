...
/*
 ...
 메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용할때 쓰는 패턴 

작성방법
1. 생성자의 접근지정자를 private으로 선언
2. 클래스 외부에서는 생성자에 접근할 수 없을므로 new를 통해 인스턴스
생성 할 수 없다.
...
class NoSingleTone{
	...
	/*
	 ...
	 선어하지 않으면 클래스 외부 혹은 다른 패키지에서는 호출할 수 없기 
	 때문에 new를 통해 인스턴스 생성 자체가 불가능하기 때문이다. 
	 */
...
class SingleTone{
	...
	int shareVar;
	/*
	  정의한 클래스와 동일한 타입으로 선언된 정적 멤버변수로 JVM(자바가상
	  머신)에 의해 프로그램이 시작될때 미리 Method영역에 로드되어 사용할
	  준비를 마치게 된다.
	 */
	...
	/*
	  ...
	  외부로 반환 한다. 이런 메서드를 "유틸리티 메서드"라고 한다.
	 */
