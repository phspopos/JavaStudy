...
/*
 ...
 -정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는 시점에 초기화 된다.
 -main() 메서드가 실행되기전에 초기화되므로 main() 메서드 샐행시 별도의
 ...
 -메서드 앞에 static을 붙이면 정적 메서드가 된다. 정적변수와 모든
 	특성이 동일하다.
 */
...
class MyStatic{
	...
	/*
	 ...
	 수 있다. 인스턴스형 멤버, 정적 멤버 모두 접근 가능하다. 
	 */
	void instanceMethod() {
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 (※ 참조변수를 통해 접근할거면, 궅이 정적변수를 사용할 이유가 
		 없기 때문이다.) 
		 */
		...
		/*
		 결과를 보듯 입력시에는 각각의 참조변수르 사용 했지만 결국 하나의
		 정적변수에 값을 입력한 결과가 되므로, 정적변수에 접근시엔는
		 참조변수를 사용하지 말고, 클래스명을 사용하는 것이 좋다.		  
		 */
		...
		/*
		 ...
		 인스턴스 참조변수를 통해서만 접근할수 있다. 
		 */
		//MyStatic.instanceMethod();
		MyStatic.staticMethod();
	/*
	 결론 : 일반적으로 인스턴스형 멤버는 인스턴스의 참조변수를 통해
