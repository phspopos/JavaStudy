...
/*
  ...
  -Array 라는 이름처럼 '배열'의 특성을 가지고 있어 index를 통해
  저장 및 접근이 가능하다.
 */
	...
	public static void main(String[] args) {
		...
		/*
		 중복저장 : List는 배열의 특성을 가지므로 데이터를 중복
		 		...
		 		있기 때문이다. add()는 추가에 성공할 경우 true를 
		 		반환한다.
		 */
		...
		/*
		 출력하기 : 
		 방법1 : 일반 for문을 사용. 이때는 인덱스를 통해 접근해야
		 하므로 get() 메서드를 사용한다.
		 */		
		System.out.println("\n [일반for문 사용]");
		...
		*/
		...
		//int index2 = list.lastIndexOf("트와이스");
		...
		/*
		 전체삭제 : 아래 2개의 메서드를 통해 컬레션에 저장된 전체
		 			데이터를 삭제할 수 있다.
		 */
		list.removeAll(list);
		list.clear();
